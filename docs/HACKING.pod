...
C<0xC0FFEE>. This ensures all operations on the array are atomic (semaphore is
...
communication. There are five stages to this fine dance:
...
6. The client browser receives the message and renders it on the page. Then
another asynchronous request is initiated and we are looped back to step 1


This technique is called HTTP long-polling. After the long poll is
initialized, the server then places the C<$c_id> (a randomly generated, unique
ID) into the shared memory location keyed by C<0xC0FFEE> only after
initializing the shared memory keyed by C<$c_id> with the last_seen
timestamp. This lets us know if a client has "timed out" and should be removed
from the list (presumably the client has recognized the time out and opened
another socket already, in which case they will be in the queue under a
different C<$c_id>).
...
to every client in the array at C<0xC0FFEE>.
