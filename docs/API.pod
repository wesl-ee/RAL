...
Submitting a subscription request will open a real-time socket to the server.
Text will be piped to you when something relevant happens (revelance is
determined by the paramters you pass to the subscription).

Currently there is no C<PING> text to keep the socket connection
alive. Unfortunately, this means you will need to re-submit the subscribe
request every ~15 seconds. This does not cause much overhead, but be
aware this is not intended behavior and will be fixed in a future release.
...
Returns a JSON object describing the message. You probably want to parse
the C<body> property after checking the message C<type>.
